 ...
 * audio_format_ctrl.c
 *
 * "Kernel space program" companion:
 * - Does NOT decode audio (no MP3/AAC/FLAC decoding in kernel)
 * - Provides a tiny control interface to store/expose the user-selected format
 *   via a misc character device and sysfs.
 *
 * This is optional plumbing; the actual decode+playback remains in user space.
 */
...
/* /dev/audio_format_ctrl: read => "mp3\n" ; write => "aac\n" or "2\n" */
...
/* sysfs: /sys/class/misc/audio_format_ctrl/format */
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Embedded Linux ALSA example");
MODULE_DESCRIPTION("Format selection control (no decoding) for user-space audio pipeline");
